...
export default {
  ...
  async execute(message, client) {
    try {
      ...
      // 2. ميزة الذكاء الاصطناعي وفحص الحالة (للـ 6 رومات)
...
// ==========================================
// ⚠️ ضع آي دي حسابك هنا في السطر التالي بين علامتي التنصيص:
// ==========================================
const ownerId = "1441891628204822629"; 

async function handleAiChat(message) {
  try {
    // آي دي الرومات الستة الخاصة بك (بدون تكرار)
    ...
    // 1. فحص حالة فهد الشمري (صاحب السيرفر) باستخدام ownerId
    let ownerStatusText = "";
    try {
      const ownerMember = await message.guild.members.fetch(ownerId).catch(() => null);
      const status = ownerMember?.presence?.status;

      if (!ownerMember || status === 'offline' || !status) {
        ownerStatusText = `⚠️ صاحب السيرفر فهد الشمري (<@${ownerId}>) **أوفلاين** حالياً.\n`;
      } else if (status === 'dnd') {
        ownerStatusText = `🔴 صاحب السيرفر فهد الشمري (<@${ownerId}>) في وضع **عدم الإزعاج (DND)**، يرجى عدم إزعاجه.\n`;
      } else if (status === 'idle') {
        ownerStatusText = `🟡 صاحب السيرفر فهد الشمري (<@${ownerId}>) في وضع **الخمول (AFK)** حالياً.\n`;
      } else if (status === 'online') {
        ownerStatusText = `🟢 صاحب السيرفر فهد الشمري (<@${ownerId}>) **أونلاين**! تقدرون ترسلون الاستفسارات حقكم.\n`;
      }
    } catch (err) {
      ownerStatusText = `👤 صاحب السيرفر: فهد الشمري (<@${ownerId}>)\n`;
    }

    // 2. جلب رد الذكاء الاصطناعي من جوجل جيمناي
    ...
    // 3. دمج حالة صاحب السيرفر مع رد البوت وإرسالها للروم
    if (aiReply) {
      const finalMessage = `${ownerStatusText}\n💬 **الرد الذكي:**\n${aiReply}`;
      await message.reply(finalMessage);
    } else {
      await message.reply(`${ownerStatusText}\nعذراً، لم أستطع فهم السؤال، حاول مرة أخرى.`);
    }
